...
export class AppHeader extends LitElement {
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.has('currentView')) {
            if (this.currentView === 'jarvis' && this.startTime) {
                this._startTimer();
            } else {
                this._stopTimer();
            }
        ...
        if (changedProperties.has('startTime')) {
            if (this.startTime && this.currentView === 'jarvis') {
                this._startTimer();
    ...
    _startTimer() {
        ...
        // Only start timer if we're in jarvis view and have a start time
        if (this.currentView === 'jarvis' && this.startTime) {
    ...
    isElapsedTimeWarning() {
        if (this.currentView === 'jarvis' && this.startTime) {
            const elapsed = Math.floor((Date.now() - this.startTime) / 1000);
            const cyclePosition = elapsed % 600; // 10-minute cycle
            return cyclePosition >= 540 && cyclePosition < 600; // 9-10 minute mark
        }
        return false;
    }
    ...
    getElapsedTimeDisplay() {
        ...
        return isWarning ? `🚨 ${elapsedTime}` : elapsedTime;
    }
