...
function sanitizeText(text) {
    ...
    // Strip any HTML tags generated by Gemini to avoid persisting markup artifacts
    const noMarkup = text.replace(/<[^>]*>/g, '');
    
    const sanitized = noMarkup
...
async function sendCombinedQuestionsToAI(combinedText, geminiSession) {
    ...
    try {
        // Append MASTER_PROMPT v2 task before sending to Gemini
        const masterPromptTask = "\n\nTask: Answer fully following MASTER_PROMPT instructions";
        const finalText = combinedText.trim() + masterPromptTask;

        // Send the augmented text to Gemini
        await geminiSession.sendRealtimeInput({ text: finalText });
...
function setupGeminiIpcHandlers(geminiSessionRef) {
    ...
    ipcMain.handle('send-text-message', async (event, text) => {
        ...
        try {
            ...
            const masterPromptTask = "\n\nTask: Answer fully following MASTER_PROMPT instructions";
            const finalText = text.trim() + masterPromptTask;
    ...
    ipcMain.handle('process-context-with-screenshot', async (event) => {
        ...
        try {
            ...
            const masterPromptTask = "\n\nTask: Answer fully following MASTER_PROMPT instructions";
            const finalText = completeTranscription.trim() + masterPromptTask;
            await geminiSessionRef.current.sendRealtimeInput({ text: finalText });
            return { success: true };
        } catch (error) {
